...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if df_wo.empty:
        ...
        # Filter WO berdasarkan filter sidebar
        df_wo_f = df_wo.copy()
        if sel_sub != "Semua" and "subsistem" in df_wo_f.columns:
            df_wo_f = df_wo_f[df_wo_f["subsistem"] == sel_sub]

        # KPI
        c1, c2, c3, c4 = st.columns(4)
        total_wo = len(df_wo_f)
        wo_pm    = (df_wo_f["tipe"] == "PM").sum() if "tipe" in df_wo_f.columns else 0
        wo_cm    = (df_wo_f["tipe"].isin(["CM","EM"])).sum() if "tipe" in df_wo_f.columns else 0
        total_dt = df_wo_f["downtime_jam"].sum() if "downtime_jam" in df_wo_f.columns else 0

        for col, label, val, color in [
            (c1, "Total WO",      total_wo,              "#1F4E79"),
            (c2, "WO PM",         int(wo_pm),            "#2D7D32"),
            (c3, "WO CM / EM",    int(wo_cm),            "#F57F17"),
        ...
        with col_chart:
            st.markdown('<div class="section-title">Distribusi Tipe WO</div>',
                        unsafe_allow_html=True)
            if "tipe" in df_wo_f.columns and not df_wo_f.empty:
                tipe_count = df_wo_f["tipe"].value_counts().reset_index()
                tipe_count.columns = ["Tipe", "Jumlah"]
                fig_wo = px.bar(
                    tipe_count, x="Tipe", y="Jumlah", color="Tipe",
                    color_discrete_map={"CM":"#1F4E79","PM":"#F9A825",
                                        "EM":"#C62828","PdM":"#2D7D32"},
                    text="Jumlah"
                )
                ...
                st.info("Tidak ada WO untuk subsistem yang dipilih.")
        ...
        with col_tbl:
            ...
            ] if c in df_wo_f.columns]

            df_wo_disp = df_wo_f[show_wo_cols].copy()
            # Format tanggal — hapus 00:00:00
            for col_dt in ["tgl_dibuat", "tgl_rencana"]:
                if col_dt in df_wo_disp.columns:
                    df_wo_disp[col_dt] = pd.to_datetime(
                        df_wo_disp[col_dt], errors="coerce"
                    ).dt.strftime("%d/%m/%Y").fillna("-")

            df_wo_disp = df_wo_disp.rename(columns={
                "wo_no": "No WO", "asset_id": "Asset ID", "subsistem": "Subsistem",
                "tipe": "Tipe", "prioritas": "Prioritas", "tgl_dibuat": "Tgl Dibuat",
                "tgl_rencana": "Tgl Rencana", "durasi_jam": "Durasi (jam)",
                "downtime_jam": "Downtime (jam)", "pic": "PIC", "keterangan": "Keterangan"
            })
            st.dataframe(df_wo_disp, use_container_width=True,
